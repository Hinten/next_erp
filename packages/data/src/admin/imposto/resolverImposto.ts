...
export function createImpostoResolver(deps: ImpostoResolverDeps): ImpostoResolver {
  ...
  async function resolveImpl(produtoUid: string, itemImposto: unknown): Promise<Imposto | null> {
    ...
    if (produtoMatch) {
      ...
      console.warn(
        `[imposto-resolver] produto '${produtoUid}': impostoProduto '${produtoMatch.id ?? '?'}' matched operação '${deps.bundle.operacaoId}' but failed impostoSchema — ${firstIssue(parsed.error)} — falling through`,
      );
    }
    ...
    if (categoriaUid) {
      ...
      if (categoriaMatch) {
        ...
        console.warn(
          `[imposto-resolver] produto '${produtoUid}': impostoCategoria '${categoriaMatch.id ?? '?'}' (categoria '${categoriaUid}') matched operação '${deps.bundle.operacaoId}' but failed impostoSchema — ${firstIssue(parsed.error)} — falling through`,
        );
      }
    }
    ...
    for (const regra of deps.bundle.regrasImposto) {
      ...
      console.warn(
        `[imposto-resolver] produto '${produtoUid}': regraImposto '${regra.id ?? '?'}' matched (operação '${deps.bundle.operacaoId}') but failed impostoSchema — ${firstIssue(parsed.error)} — falling through`,
      );
    }
    ...
    if (operacao) {
      ...
      /* eslint-disable-next-line no-console -- E4: the base config allows only
         warn/error, and promoting this line to `warn` is exactly what the three
         lines above forbid. The same warning is pre-existing on the file this
         was moved from; carrying it into packages/data would make every future
         commit that stages this file fail lint-staged's --max-warnings 0. */
      console.debug(
        `[imposto-resolver] produto '${produtoUid}': operação '${deps.bundle.operacaoId}' carries no usable default Imposto — ${firstIssue(parsed.error)}; no imposto resolved`,
      );
    }
