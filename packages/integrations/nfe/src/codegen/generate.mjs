 ...
 *
 * Reads one vendored SEFAZ XSD pack and emits its typed module. Two packs:
 *
 * - `moc` (default) — `generated/moc<ACTIVE_MOC>/schemas/` →
 *   `generated/moc<ACTIVE_MOC>/types/nfe-schema.ts` plus the Zod mirror below.
 * - `conscad` — the Consulta Cadastro layout 2.00 XSDs in `generated/conscad/`
 *   → `generated/conscad/types/conscad-schema.ts` (no Zod mirror).
 *
 * Not `src/types/` — the files there are re-export shims the generator never
 * writes to.
 *
 * The emitted file holds TypeScript interfaces for every complexType plus
 ...
 *
 * Zero dependencies (built-in minimal XML parser). `gen:nfe-types` runs this
 * script once per pack. Generated output is committed; re-run only when the
 * XSD packs change. See ADR 0004.
 */
...
const MOC_DIR = join(GENERATED_DIR, `moc${ACTIVE_MOC}`);
const CONSCAD_DIR = join(GENERATED_DIR, 'conscad');

// ⚠️ Each pack is generated in its OWN process (`--pack`), so the module-level
// registries below — the complexType map, the root list, the xs:choice counter —
// never hold two packs at once. That isolation is load-bearing: the consCad
// leiaute and the NF-e leiaute BOTH declare a `TEndereco`, and one shared
// registry resolves that name by file order, so the emission `enderDest` META
// would be whichever file sorts last (issue #251). Never merge packs into one
// schema dir or one run.
const PACKS = {
  moc: {
    schemaDir: join(MOC_DIR, 'schemas'),
    outFile: join(MOC_DIR, 'types', 'nfe-schema.ts'),
    outZodFile: join(MOC_DIR, 'types', 'nfe-schema-zod.ts'),
    // Header paths are relative to the emitted file.
    source: '../schemas/',
    provenance: '../schemas/MANIFEST.json',
  },
  conscad: {
    schemaDir: CONSCAD_DIR,
    outFile: join(CONSCAD_DIR, 'types', 'conscad-schema.ts'),
    outZodFile: null,
    source: '../*.xsd',
...
const PACK_NAME = packFlag === -1 ? 'moc' : process.argv[packFlag + 1];
const PACK = PACKS[PACK_NAME];
if (!PACK) {
  console.error(`Unknown --pack '${PACK_NAME}'. Known packs: ${Object.keys(PACKS).join(', ')}.`);
  process.exit(1);
}
const SCHEMA_DIR = PACK.schemaDir;
const OUT_FILE = PACK.outFile;
const OUT_ZOD_FILE = PACK.outZodFile;
...
// ---------------------------------------------------------------------------
// Emit generated/moc<ACTIVE_MOC>/types/nfe-schema.ts
// ---------------------------------------------------------------------------
const propName = (s) => (/^[A-Za-z_$][\w$]*$/.test(s) ? s : `'${s}'`);
const out = [];
// Paths in the header are relative to the emitted file (generated/mocX.Y/types/),
// so they stay correct across a MOC bump without touching this line.
out.push(`/* GENERATED by src/codegen/generate.mjs from ${PACK.source}. DO NOT EDIT. */`);
out.push(`/* Source XSD packs: ${files.length} files — see ${PACK.provenance}. */`);
out.push('');
...
// A pack without a Zod mirror (conscad — nothing validates its values at
// runtime) stops here.
if (!OUT_ZOD_FILE) process.exit(0);

// ---------------------------------------------------------------------------
// Emit generated/moc<ACTIVE_MOC>/types/nfe-schema-zod.ts
//
// Mirrors every complexType from nfe-schema.ts as a Zod schema so
