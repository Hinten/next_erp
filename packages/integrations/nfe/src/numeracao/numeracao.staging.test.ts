...
describeOrSkip('numeração — live Firestore concurrency contract', () => {
  ...
  it(
    'nextNumeracaoBulk(5) × 10 parallel → contiguous {1..50}',
    async () => {
      const filialId = newFilial();
      const { path, store } = await seedAndStore(filialId);
      try {
        ...
        for (const r of results) {
          ...
          expect(sorted).toEqual([sorted[0], sorted[0]! + 1, sorted[0]! + 2, sorted[0]! + 3, sorted[0]! + 4]);
        }
  ...
  it(
    'nextIdLote × 50 parallel → exactly {1..50}, independent of nNF counter',
    async () => {
      const filialId = newFilial();
      const { path, store } = await seedAndStore(filialId);
      try {
