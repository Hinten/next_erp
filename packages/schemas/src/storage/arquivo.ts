...
export const arquivoSchema = z
  .object({
    ...
    // Soft-delete marker (microseconds-since-epoch). Set by eager owner-media
    // triggers and by `onMensagemDeleted`; cleared when the ref is re-added or
    // the sweep finds another live reference. `sweepMarkedForDeletion` deletes
    // only after a short grace and an owner/global-mensagem recheck, so a missed
    // unmark never loses a live file.
