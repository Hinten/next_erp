...
const INVENTARIO = {
  ...
  'apps/functions/src/pedidos/sincronizarBloqueioIncidente.ts':
    'One site, class A, and the class is what makes the denorm trustworthy (#1322). It keeps `pedido.disputaAbertaEm` / `devolucaoAbertaEm` / `bloqueiosLiberados` in step with the `incidentes` subcollection, and those three fields GATE despacho, NF-e emission and the advance to `finalizado` — so a stale verdict here does not merely mislabel a pedido, it ships one that is about to be refunded. Nothing from the event payload reaches the write: `before`/`after` are read ONLY by the fast-path skip, and the value written is folded from a fresh `tx.get` over the whole subcollection plus the pedido, so an OCC retry recomputes from scratch. ML fans out several deliveries per claim (`claims` + `claims_actions` + the `/actions-history` sub-resource), so two incidentes of one pedido changing at once is the ordinary case rather than the edge. Tier 0 by convergence rather than by comparison: the fold is a pure function of the stored incidentes, so whichever attempt commits last writes the same answer — there is no clock to compare and no loser to detect. The write is a bare `tx.update` of exactly those three keys (never a converter merge, which would full-parse the patch and let the merge mask overwrite stored siblings) and is skipped entirely when the fold equals what the tx-fresh pedido already holds, so a no-op cannot churn the modification history. `ultimaModificacao` is deliberately NOT stamped — derived state bumping it would raise a false “Pedido alterado” conflict in the editor over fields the operator can neither see nor edit (#972).',
  ...
  'apps/mercado-livre/lib/marketplace/conta/testUserStore.ts':
    'One site (`put`), class A, and it exists to make an overwrite UNREPRESENTABLE rather than merely unreached. The pair bootstrap writes the ML test user at the bare role doc id; the value it protects is a password ML shows once and never reissues, so a lost write here is unrecoverable in the strongest sense in this repo. It was a bare `set`, safe only by the argument that `reutilizavel` had just read the role as absent — an argument outside the write, one refactor from being false. The guard is now re-derived from the write’s own `tx.get`: an existing document holding a DIFFERENT `id` throws `ML_USUARIO_TESTE_DUPLICADO` (tier 3, and the route turns it into a 409), while the SAME `id` re-writes identical content, which is what keeps a partial pair re-run idempotent. ⚠️ `parseRead` is soft — an unparseable document comes back raw, so `id` may be absent and the comparison fails; that is the safe direction, since a document we cannot read is one we must not replace. The additional mint takes the other route entirely (`create` at `${role}-${mlUserId}`, tier 0 — no read, nothing to race), so neither writer can land on the other’s document.',
  'apps/mercado-livre/lib/marketplace/pedidos/orderImport.ts':
    'Five sites, all re-deriving from `tx.get`. The #1087 estado-PROMOTION arm rides the pago site and stays class A — the ladder position, the target estado and the ML-order-clock comparison all come off the same `tx.get` snapshot, and the pago re-check it falls through to is evaluated against the promoted value rather than the stale one. `:654` / `:752` write the cliente / endereço outer-ref only when the fresh doc still lacks it; `applyFreteSemEnvioStep` owns `freteInicial` for an order with no Mercado Envios shipment and is class A on BOTH its arms — every guard and every written value comes off the same `tx.get` snapshot. Its SEED arm (guards: block already present, endereço not yet resolved) stays create-only, so a concurrent writer that got there first simply wins. Its MONEY-RECONCILIATION arm (#1087) is not create-only — it recomputes `valorCobrado` from the same `tx.get` snapshot on EVERY invocation, because a sem-envio PACK reaches no frete conference and would otherwise hold every sibling\'s items against `orders[0]`\'s money for ever (#791\'s failure mode on the one path #791 did not cover). Class A regardless: the target comes from `derivePedidoFreteTotals` over the fresh doc\'s own `itens`/`descontoTotal`/`freteInicial`, nothing is captured before an await, and a converged pedido writes nothing. ⚠️ It writes MONEY ONLY and must stay that way — it shares a transaction with the stock-moving arm below, so a repair that also touched `freteInicial.estado` would be a second opinion on an irreversible movement. Its DELIVERY-ADVANCE arm (`fulfilled` → `entregue`, #1087) is the other write here that is not create-only, and it moves PHYSICAL STOCK (`entregue` ∈ `ESTADOS_FRETE_REMOVE_ESTOQUE`), irreversibly — `efeitoEstoquePedido` holds on the pedido estado once applied, so reverting the frete restocks nothing. Five guards, all tx-fresh: `entregue` is the only value it can emit (so `fulfilled: false`/null writes nothing and no downgrade is expressible); it returns early when the block already reads `entregue` (idempotence — ML sends several deliveries per order and the sweep replays them); it is tier 2 in **µs** via the shared `payloadNaoEhAntigo(freshPedido.lastMarketplaceUpdate, orderLastUpdatedUs)` — the same helper and unit as the estado promotion and downgrade, deliberately not a second comparison; it refuses a block that is no longer the un-arranged FOB seed (`integracaoFreteOuterRef`/`externalOptionIntegracao`/`printLabelId`/`externalId` — a SECOND WRITER owns `estado` from then on, and taking it would latch the Melhor Envio webhook out of its own label permanently via that route\'s `TERMINAL_ESTADOS`, besides moving stock early from a pre-removal estado); and it refuses `ESTADOS_FRETE_TERMINAIS_SEM_ENVIO`. ⚠️ Those last two answer DIFFERENT questions — "does someone else own this block?" versus "did a human close it?" — and the first review of #1289 caught them being conflated, which is the failure mode to watch for if either is ever widened. Needed despite being a "promotion" because `fulfilled` is FEEDBACK and feedback is editable, so a replayed `true` can contradict a newer `false`. Precedence is structural rather than a guard: the arm is reachable only when the order has no shipment at all, so `fulfilled` can never override a shipment-derived `EstadoFrete`; the divergence site runs a READ PHASE then decides the verdict on tx-fresh inputs only (its `veredito` is reset per attempt — legacy poisoned retries with that flag); the pago site re-derives the actually-missing pagamento set from its own reads, so the pre-fetched candidate pool may safely be a superset. The two sites #776 cited as broken were fixed here by #791.',
  ...
  'apps/web/lib/pedidos/incidentePort.ts':
    "B — same port shape as `listingPort` (the patch builder runs on the `tx.get` snapshot inside the callback), but filed here because ONE input is captured outside and has to be: `saveIncidenteEdit`'s `baseline` is the version of the incidente the operator actually reviewed, which is the whole point of the guard — re-reading it at save time would compare the document against itself. The named guard is `detectIncidenteConflict(baseline, current, pretendido)` against the tx-fresh doc; it throws `IncidenteConflictError` (an authored field this save writes moved remotely) or `IncidenteMissingError` (deleted mid-edit), because the browser SDK has no `lastUpdateTime` precondition (`apps/web/CLAUDE.md` rule 3). ⚠️ The written patch is the INTERSECTION of the judged patch and a re-derivation against the fresh doc, and the first review of this file caught why both halves are needed: values from the re-derivation (so a resolução that LOCKED since the baseline is dropped and the live `resolucao.frete` rides along), keys from the judged one (an authored field the operator left alone is absent from the verdict, so re-derivation alone would write the form's stale value back over whoever changed it — unjudged and invisible in the conflict modal). It replaces a whole-document converter `set` built from a copy captured when the editor opened, which regressed `resolucao` / `claimStatus` / `claimStage` / `entregue` — the fields the Mercado Livre claims webhook merges onto the same document (#1250). ⚠️ `now()` is MICROSECONDS here (`incidente.ultimaModificacao` is `microsSinceEpoch`), unlike the ms clock the ML link port it is modelled on uses. The write is a bare `tx.update` of the authored keys only, never a converter merge, which is also what keeps the server-owned `overrideBloqueio` out of `affectedKeys()`.",
  ...
  'apps/shopee/lib/shopee/core/tokenStore.ts':
    'MIXED, filed here. THREE sites, one per phase of a leased refresh — the repo’s only lease, which ADR 0011 permits for token refresh alone because Shopee’s refresh token is single-use and rotating, so two instances that both spend it can BURN the pair rather than merely write twice. `acquire` is class **A**: every verdict (absent / fresh / held / acquired) is re-derived from the one `tx.get` inside the callback, and the two exclusions are different mechanisms — OCC arbitrates two callers that read the same version (the loser ABORTS, re-runs and comes back `held`), while the stored lease is what tells the caller arriving AFTER that commit that a refresh is in flight. The lease EXPIRES (30 s) and is **never renewed**, deliberately below the 60 s refresh skew so a crashed or hung refresher is taken over while the old token is still nominally alive; a corrupt lease reads as NO lease (ADR 0011’s wrong-way default — a hand-edited document must not freeze an account), and `nowMs === expiraEm` is expired and takeable. `commit` is class **C** — the pair comes from `POST /api/v2/auth/access_token/get` — with the named guard "the stored `refresh_token` must still equal the one we spent, else DROP our pair and return the STORED access token". That guard is an IDENTITY comparison, not a clock, so rule 7’s cross-unit trap cannot apply to it; it keys on the TOKEN rather than on the lease because a lease that expired and was re-taken by an instance that has not written yet still leaves OUR pair the live one. The write is `tx.update`, never `set`: the refresh response echoes no `shop_id_list`/`merchant_id_list` and a full write would wipe them; the patch is built with `parseMerge` BEFORE the transaction opens, so a pair the CREDENTIAL schema rejects fails on the release path instead of inside the callback with the lease held (⚠️ not a blank `refresh_token` — the package rejects those against `shopeeRefreshResponseSchema` one surface earlier; what can still reach that parse is a pathological `expire_in`). The drop branch hands the lease back when it is still OURS — reachable when our lease lapsed, a second instance re-took it, spent the same refresh token and committed first, leaving a lease it did not own standing. `releaseOrAdopt` is class **C** too, and its ORDER is the guard, twice over: it re-reads, and a stored refresh token that is no longer the one we spent means a NEWER pair landed (another instance, or an operator re-consenting), so it adopts that token and stamps NOTHING — reached BEFORE any terminal verdict, so a reauth code answered against a superseded token cannot disconnect a conta that is in fact healthy, and BEFORE the ownership test, so a lapsed-and-re-taken lease cannot turn that same newer pair into a rethrown provider failure (a consent clears the lease outright, so the adoption that matters most arrives with no lease at all). Only a terminal failure is stamped (`ultimaFalhaRefresh`, written wholesale as a nested map because a top-level key on `update` replaces the map, which is what a "newest failure" stamp wants); transient ones release without a trace so the panel cannot flap. ⚠️ This does NOT contradict the "comment only" entry for `apps/mercado-livre/lib/marketplace/core/tokenStore.ts` below, and the difference is worth stating because the two look like the same problem: there the refresh POST would sit INSIDE the callback, so an OCC retry re-fires the non-idempotent grant. Here the grant is spent BETWEEN transactions — `acquire` closes before the call and `commit` opens after it — so no retry can ever re-send it, which is precisely what makes a transaction affordable at all.',
  'apps/shopee/lib/shopee/pedidos/orderPedidoTx.ts':
    'C — ONE site, and the class is the honest one rather than the flattering one (#1513, step 5). The WHOLE mapped pedido body is built OUTSIDE the callback from `get_order_detail` (+ `get_escrow_detail`) and re-applied verbatim on an OCC retry, which is exactly class B, widened by two network calls. (Mercado Livre’s own `orderPedidoTx.ts` is filed A because it maps INSIDE the callback; this one deliberately does not, so a retry cannot re-run a mapper over a payload that no longer matches the clock it was fetched with.) Tier 2 in **µs**, and the UNIT IS THE GUARD: `get_order_detail.update_time` is SECONDS, so it is converted ONCE by `microsDeSegundosShopee` (= `update_time × 1_000_000`) and NEVER by `coerceToMicros`, which classifies by magnitude and would read `1.78e9` as MILLISECONDS and answer 1970 — a comparison that says “older” for ever (`orderMapping.test.ts` pins that near-miss against `coerceToMicros` itself). The named guard: re-read the pedido with the callback’s own `tx.get`; compare the stored `lastMarketplaceUpdate` (through `coerceToMicros` — the legacy corpus holds ms ints and ISO strings THERE) against the incoming watermark; DROP with the named outcome `ignorado-obsoleto` when strictly older; and re-derive every written value from that same snapshot — the estado through `estadoShopeeAplicavel(atual.estado, alvo)`, the `hasUserInteraction` freeze on the data group, the per-field fill-once checks read off the RAW snapshot, the append-only item merge and the freight merge. Accept is `>=` rather than `>` for two independent reasons: the ML convergence argument, and Shopee’s 1-second resolution (`UNPAID → PENDING → READY_TO_SHIP` inside one second share one stamp, so `>` would drop the last two for ever). Re-mapping an EQUAL stamp is safe because the mapper is pure over the wire payload: a replay is byte-identical, every field comparison answers “same”, the patch comes out EMPTY and nothing is written (`ignorado-sem-mudanca`, asserted with an empty `opLog`). ⚠️ There is NO generic deep-equal here — every comparison is field by field and strict, because a fold decides which edits count as “no change” and therefore which ones are silently never written (#1372). The watermark is written as the ACCEPTED value rather than `max(stored, incoming)`: with `>=` accepted those are the same number, and a second comparison is only a second thing that can disagree with the gate. `ultimaModificacao` IS `max(stored, nowUs)` and never null (`parseMergePatch` keeps a null and would erase the stamp). The absent-document branch is `tx.create`, never `tx.set`: OCC turns a concurrent create into an abort-and-retry that falls through to the update path, and if the engine ever failed to detect it, `create` fails loudly instead of clobbering the winner. `capturaComprador` is a DIARY, never a guard — the only things read back are `tentativas` (a counter) and the `expirado` LATCH, and `tentativas` counts distinct order updates rather than deliveries, so a Cloud Tasks retry cannot inflate it or make a replay look like a change. This module moves NO stock: `onPedidoEstoqueSync` owns that and reacts to the `estado` this transaction writes. Pinned by `orderPedidoTx.test.ts` — the competing-writer race (exactly one abort, one create), the equal-stamp replay (stored doc deep-equal, empty op log), the older-stamp drop, `pago → aguardandoConfirmacaoDePagamento` refused while the watermark still advances, `cancelado → pago` allowed and logged, the fill-once that never unlinks a cliente, and the `expirado` latch.',
  'apps/shopee/lib/shopee/pedidos/pagamentoTx.ts':
    "B — ONE site, the SECOND transaction of the Shopee order import (#1514, step 6). The mapped pagamento bodies are built OUTSIDE the callback by `mapearPagamentosShopee`, from the same `get_order_detail` + `get_escrow_detail` pair `orderPedidoTx.ts` already spent, and re-applied verbatim on an OCC retry. It is filed B rather than C because the two Shopee calls are NOT between this read and this write — `salvarPedidoShopee` closes first and this transaction opens after it, so the widest window here is one Firestore round trip, not a network one. It is deliberately NOT folded into `salvarPedidoShopee`: that transaction is the PEDIDO's, one OCC abort would re-run two mappers over one payload, and the pagamento write must be able to run on a pedido that came out `ignorado-sem-mudanca` (the escrow carries no clock of its own, so the money can move while the order row does not). The named guard is the PEDIDO's watermark, re-read with the callback's own `tx.get`: the stored `lastMarketplaceUpdate` — through `coerceToMicros`, because the legacy corpus holds ms ints and ISO strings THERE, and NEVER `microsDeSegundosShopee`, which is for the SECONDS side — is compared against the incoming one in **microseconds**, tier 2, accepting `>=` for the same two reasons step 5 does (ML's convergence argument, and Shopee's 1-second resolution). Strictly older DROPS with the named outcome `ignorado-obsoleto`. A pedido that does not exist drops with `ignorado-sem-pedido` — a subcollection under a missing ancestor is legal in Firestore, which is exactly why it is refused here rather than tolerated. Everything written is re-derived from that snapshot: `congelado` from the pedido's own `hasUserInteraction`, the status ladder from the STORED `status_pagamento` through `statusPagamentoAplicavel`, and the fill-once tests off the RAW stored pagamento. The SECOND read is `tx.get(pagamentoCollection.ref(db, { pedidoId }))` — the whole subcollection, the atomic read the client SDK cannot do (`packages/data/src/admin/pedidoReconcile.ts:302` is the precedent) — and it is not optional: a BR combined payment fans out to N docs, Shopee stops returning `payment_info` after READY_TO_SHIP, and a later delivery that re-took the primary's `valor` while an earlier delivery's secondaries still stood would put Σ pagante at nearly twice the nota. On a marketplace `canalDevolveTroco` is FALSE, so that is a hard SEFAZ 865/866 forever, not a warning. Which stored docs are OURS is decided by recomputing `makePagamentoIdShopee(contaId, orderSn, sufixoPagamentoShopee(i))` for `i` up to `MAX_PAGAMENTOS_COMBINADOS_SHOPEE`, never by the `id` FIELD (that one rides `...base` through the operator's form and is therefore reachable by an edit) — which is also what keeps the legacy `-desconto` sibling out of this transaction entirely. The absent-document branch is `tx.create`, never `tx.set`. There is NO generic deep-equal: every comparison is field by field and strict, because a fold decides which edits count as \"no change\" and therefore which ones are silently never written (#1372) — and here `onPagamentoChanged` ignores only `id` and `ultimaModificacao`, so ANY other re-stamped field writes a `historicoDeModificacoes` row on every push. A byte-identical replay therefore has to produce a genuinely EMPTY patch: `marketplace.atualizadoEm` is stamped with the ORDER clock (`watermarkUs`), never with `nowUs`, and `ultimaModificacao` is appended only to an already non-empty patch. `liquidacao` is the WEEKLY SETTLEMENT SWEEP's top-level field and never appears in a patch this transaction writes, which is what lets the two writers own disjoint masks of one document. Nothing here is ever DELETED: a pagamento the operator removed is recreated on the next delivery (the money really moved), and a surplus doc from a richer earlier delivery keeps its own `valor`. Pinned by `pagamentoTx.test.ts` — the older-delivery drop, the byte-identical replay (empty op log, no `update`), the two racing pagamento writes, the sweep-write-vs-task-write race on disjoint field groups, the fill-once/frozen/lifecycle table, and the Σ pagante == `valorCobrado` cross-module check.",
  'apps/shopee/lib/shopee/pedidos/liquidarPagamento.ts':
    "B — ONE site, the weekly escrow settlement (#1514, step 6). The escrow BODY (`get_escrow_detail`) and the `get_escrow_list` row are both fetched OUTSIDE the callback and reach the write, so an OCC retry re-applies them verbatim; it is B rather than C only because the sweep is not a request path — the reads are one tick's own, seconds old, and the caller loops rather than waiting. **Two named guards, both re-derived from the callback's own `tx.get`.** (1) The release-time watermark, tier 2 in **µs**, and the unit IS the guard: `escrow_list.escrow_release_time` is SECONDS and is converted ONCE by `microsDeSegundosShopee` (= `× 1_000_000`) and NEVER by `coerceToMicros`, which classifies by magnitude, reads `1.65e9` as MILLIseconds and answers 1970 — a settlement that says \"older\" for ever (`liquidarPagamento.test.ts` pins that near-miss directly against `coerceToMicros`). The stored side is read off the RAW snapshot at `liquidacao.escrowReleaseTimeUs`; strictly NEWER stored ⇒ `ignorado-obsoleto`, EQUAL falls through to (2), and a null on either side falls through because an absent release time is not evidence of order. (2) Field-by-field content equality over the sweep-owned money keys; identical ⇒ `ignorado-sem-mudanca` with **ZERO writes**, which is what makes the one-day weekly overlap idempotent — `onPagamentoChanged` ignores only `id` and `ultimaModificacao`, so any write at all would file a `historicoDeModificacoes` row per conta per week for ever. ⚠️ `liquidacao.liquidadoEmUs` is deliberately EXCLUDED from that comparison: it is `nowUs` and comparing it would make the no-change branch unreachable. There is NO generic deep-equal — every comparison is written out, per #1372. The write is `tx.update` with `parseMerge`, never `tx.set`; `liquidacao` is this sweep's own TOP-LEVEL field (the task path never names it), and `marketplace` is REBUILT from the tx-fresh `raw.marketplace` with the escrow diary spread over it because `update()` masks at a top-level key and replaces the map. The money figures come from the SAME pure `tarifasDeShopee`/`diarioMarketplaceDeEscrow` the import mapper uses — imported, never re-derived — so a sweep write and a task write racing the same pagamento agree by construction; `tarifas` is `.min(0)` on the schema and that clamp lives in the mapper, not here. The sweep writes NO `valor`, `status_pagamento`, `forma_de_pagamento`, `parcelas` or `cartao`, so `Σ vPag == vNF` cannot move, and it touches no pedido, no `estado` and no stock. The cursor document `liquidacaoShopee/{integracaoId}` is written OUTSIDE this file with one merge per conta per tick and runs no transaction of its own. ⚠️ BOTH guards and the patch itself live in ONE pure function, `preverLiquidacaoShopee(raw, …)`, which the callback runs on its own `tx.get` snapshot and the rehearsal CLI's dry run runs on a plain read — so `liquidar:pagamentos --dry-run` prints the exact patch a live tick would write instead of a second implementation's opinion of it, and a rehearsal that disagreed with production would be worse than none.",
  ...
  'apps/mercado-livre/lib/marketplace/pedidos/pedidoTravadoSweep.ts':
    'C — an ML `GET /orders/{id}` sits between the candidate query and the write, which is the widest window in this folder: the weekly sweep decides on a read taken minutes earlier. Every input is therefore RE-DERIVED inside the callback from its own `tx.get` — estado still in `ESTADOS_PEDIDO_TRAVADO`, `timestamp` still past the horizon, `hasUserInteraction` still unset, `lastMarketplaceUpdate` still present, and no pagamento `aprovado` — and any disagreement returns `mudou-durante` without writing. That guard is what stops a `payments` notification approving the sale inside the window from being clobbered, and it is mutation-proven in `pedidoTravadoSweep.test.ts`.',
  ...
  'apps/mercado-livre/lib/marketplace/preco/precoSync.ts':
    'One site (`finalizePriceSyncJob`, #1144), class **B**, and the mass-import argument above applies verbatim with a bigger cast: `status` had FIVE writers that did not coordinate — the stale-orphan reclaim in `startPriceSyncJob`, `failJob`, the `completed` flip, the final-attempt `stampFalhaTerminal`, and the `atualizar-precos` start route’s enqueue-failure fallback — and the operator’s `atualizar-precos/cancelar` is a sixth, landing at any moment including mid-dispatch. All six now funnel through this one transaction, so it is the SOLE writer of terminal status. The guard is named and explicit: `status` (still `running`?) and `integracaoId` (the ownership check, opt-in via `expectIntegracaoId`) are both re-derived from the `tx.get` snapshot, and a concurrent winner turns the call into a `not-running` no-op in EITHER direction — a late `completed` cannot bury a cancel, and a late cancel cannot bury a `completed`. ⚠️ Everything the patch takes FROM the read goes through `doSnapshot` and is computed inside the callback — `filaRestante` off `job.fila.length`, and the synthetic `JOB_INTERROMPIDO`/`JOB_CANCELADO` row’s shard index off `job.relatorioLinhas` — because a cursor captured before the transaction is exactly the stale value an OCC retry would re-apply over the winner. That row rides the SAME transaction as the stamp, so the two cannot half-land (it was a `db.batch()` for that reason before this). `checkpoint()` is deliberately OUTSIDE: it writes no `status`, so it cannot clobber a terminal state, and putting a per-item write behind an OCC retry loop inside a 540s worker would buy nothing.',
  ...
  // ---- Test harness -------------------------------------------------------
  'apps/shopee/lib/shopee/testing/fakeDb.ts':
    'The Shopee app’s shared in-memory Firestore double. Not a call site — it ADAPTS the shared `OccEngine` onto the Admin-SDK shape (`db.runTransaction(cb)`) in five lines, exactly as the engine’s own header describes, and supplies the `applyWrite` that throws gRPC 6 on `create` over an existing document and gRPC 5 on `update` of an absent one. Without those two throws every `tx.create` assertion in `orderPedidoTx.test.ts` would be vacuous. It is in this inventory’s scope because the guard greps raw TEXT, which is also why the file’s header names the method indirectly nowhere and directly here.',
