...
const INVENTARIO = {
  ...
  'apps/web/lib/pedidos/incidentePort.ts':
    "Same port shape as `listingPort` — the patch builder runs on the `tx.get` snapshot inside the callback, so the conflict verdict, the resolução LOCK state and every written value all come off the same read and an OCC retry recomputes from scratch. Tier 3: `saveIncidenteEdit`'s `patchFor` throws `IncidenteConflictError` (the doc changed on a field this save writes) or `IncidenteMissingError` (deleted mid-edit) to abort, because the browser SDK has no `lastUpdateTime` precondition (`apps/web/CLAUDE.md` rule 3). It replaces a whole-document converter `set` built from a copy of the incidente captured when the editor opened, which regressed `resolucao` / `claimStatus` / `claimStage` / `entregue` — the fields the Mercado Livre claims webhook merges onto the same document (#1250). ⚠️ `now()` is MICROSECONDS here (`incidente.ultimaModificacao` is `microsSinceEpoch`), unlike the ms clock the ML link port it is modelled on uses. The write is a bare `tx.update` of the authored keys only, never a converter merge, which is also what keeps the server-owned `overrideBloqueio` out of `affectedKeys()`.",
