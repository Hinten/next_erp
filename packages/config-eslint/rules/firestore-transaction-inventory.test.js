...
const INVENTARIO = {
  ...
  'apps/mercado-livre/lib/marketplace/pedidos/orderPaymentImport.ts':
    'C — three ML API calls precede the transaction. The #1087 bootstrap guards run BEFORE it and add no writes: they only decide whether the caller may ask the `orders_v2` topic for a pedido, and the pedido itself is created by `orderPedidoTx.ts` under its own `tx.create`. Tier 2 in **µs**: reads the pagamento, the pedido and the whole pagamento collection, then drops with `skipped: "stale"` unless the incoming `ultimaModificacao` is strictly newer than the stored one. The estado advance is computed only inside the write branch.',
