 ...
 * can't run under the Admin SDK). Lives on the `@delfrance/storage/admin`
 * subpath so `firebase-admin` never leaks into the client `.` barrel. Originally
 * shipped as a local copy in `apps/mercado-livre` for the ML photo import (#439);
 * extracted here (#449) so client + server uploaders share one create-first core.
 *
