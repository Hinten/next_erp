...
- **Adding a `test/` or `scripts/` directory to a package? Check it is in BOTH
  ...
  them in surfaced 82 real findings, one of which is a live disagreement between
  code and schema: `numeracao/firestore-adapter.ts` writes
  `timestamp: new Date().toISOString()` — a STRING — into a field
  `nfeConfigSchema` declares as `millisSinceEpoch()`. Nothing failed because
  that reader is tolerant and coerces it back, which is exactly why neither
  gate nor any test ever said so.
