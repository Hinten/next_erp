...
- `docs` (:3003) Astro Starlight (hosts the ADRs) · `example` OSS demo — **not**
  Next either, a plain `tsx` script: `pnpm --filter @delfrance/example demo`, no
  dev server. ⚠️ **:3002 is free** — it was `webchat`, the embeddable widget,
  which was **dropped on 2026-09-07** and will not be ported (#153, #558 closed
  as not planned). The `site` origem it produced STAYS in `conversaSchema`: the
  imported legacy corpus carries `site` conversas and `origem` defaults to it.
