...
⚠️ The domain pipelines `ci-{nfe,freight,storage,rules,mercado-livre}.yml` are
still `paths:`-filtered and still publish nothing when they skip. That is a
deliberate remaining exception, not a model to copy — their offline suites are
covered by `ci.yml`'s full-graph lint/typecheck/build. `ci-mercado-livre.yml` is
the one whose filter names `packages/integrations/mercado-livre/**`, so a PR
touching only that package still gets an integration-level Firestore check
(not an e2e).
