...
Detail lives where it is cheaper: the per-app `AGENTS.md` files, the
skills in `.Codex/skills/`, the ADRs under `apps/docs/`, and package READMEs.
...
1. ⚠️ **Never put a `paths:` on a lane's `pull_request:`, and never pin a check
   ...
   not a skip, _nothing_ — and a job skipped by `if:` publishes `skipped`, which
...
5. ⚠️ **The workflow YAML comes from the MERGE REF, the checkout from the PR
   ...
   the script `node` exits 1 (`Cannot find module`) _before_ the script's own
...
`[master, main, production, 'Codex/**', 'feat/**', 'fix/**']`. That key matches
...
1. **Firestore is ENTERPRISE edition — the usual limits do not apply, and it
   cuts both ways.** A query never needs an index to _run_: an unindexed
...
2. **The Firestore rulesets are GENERATED — never hand-edit, never deploy.**
   ...
   **both** `gen:rules` **and** `gen:rules:e2e` (each writes one file) and
   ...
   `ci-rules.yml` drift-checks both rulesets _and_ diffs both snapshots, so
...
3. **Codegen is minimal — exactly two generators.** The rulesets above (ADR 0003) and the NF-e XSD→TypeScript types (`gen:nfe-types` in
...
7. **Every write can lose a race — decide what happens when yours is the
   ...
   after the cutover this app is the _sole_ writer of the real data, which makes a
   ...
   predicate against a binding read _outside_ the transaction is not a guard:
...
8. **The production data has not moved yet — everything here runs on staging.**
   ...
   **off** in favour of this one. What _does_ survive it is the legacy **data**:
   ...
   you work: anything your change needs _done_ to real data or real
   ...
   app's own later writes — it is the sole live writer on the _source_ project
   ...
   it from **#1208** — the tracker that folds every window _operation_ into one
   ...
   **#899**–**#908** deploy-shaped, **#869** backfill-shaped. ⚠️ What is still
   _open_ on the label is deliberately **not** window work: code that must merge
   first (#96, #173, #829) and decisions that gate the day (#1115, #163). So the
   label is no longer the checklist — if yours is something to _run_, it is a
...
- **Porting a query from `.old/`? Re-derive it, don't transcribe it.** The
  ...
  what the query _means_ and how you would express it today.
  ⚠️ Then check that answer against rule 1, because a modern shape is **not**
  automatically cheaper. A second inequality is a _post-filter_: Firestore's docs
...
- **Repeated read of a slow-changing doc/query** on a server surface →
  ...
  via the `firestore-read-cache` skill; TTL is mandatory and _is_ the staleness
...
- **New CRUD screen** (`TableView` + `ObjectView`) → the `schema-driven-crud`
  skill. **New page or form in `apps/web`** → `apps/web/AGENTS.md`.
...
- **Does your change need something _run_ against production data or infra?**
  ...
  **`needs-migration-window`** (plus a `task:` label, usually `ops-deploy`), link
  ...
  in the issue _why earlier is wrong_. Shape: the closed-but-readable #899–#908
...
- **Re-implementing a rule that already runs on another surface? Extract it to
  ...
  OTHER copy does is the smell**, and the copies drift _toward plausible_, so
  ...
  module labelled _vincula_. Reviewers cannot diff two files by eye; the compiler
...
- **Changing the shape of data that already exists? Write a one-time migration
  ...
  buy one thing — _surviving indefinitely without a cutover_ — and there **is** a
  ...
  _and it still needs a backfill_. The script is strictly less machinery. Follow
  ...
  the legacy app keeps writing the _source_ project until the window switches it
  off, so a run before the window is partially superseded and the authoritative
  run is the one _inside_ it. ⚠️ **Staging data does not
  need to migrate.** The window moves _production_ data; staging is disposable and
...
- **Writing something that decides whether two values are "the same"? Test the
  fold's SCOPE, not just that it applies.** A _fold_ is any transform whose
...
- **Adding a `test/` or `scripts/` directory to a package? Check it is in BOTH
  ...
  `ignores` _and_ outside its tsconfig `include` (`src/**/*.ts`), so **55
...
- **Shared dependency versions live in the pnpm `catalog:`**
  ...
  **`next` in the 8 `apps/*/package.json` that have an `apphosting.yaml`** —
  ...
  unparseable as a _version_, killing the build with `CVE-2025-55182:
Vulnerable Next version catalog: detected. Deployment blocked.` A **false
  ...
  manifest: the buildpack `Override`s `FRAMEWORK_VERSION` _after_ computing it,
  ...
  reference: the catalog is still where a bump _starts_, but it is **9
  ...
  **SOLE** remaining `catalog:` consumer — `apps/webchat` was the other, deleted
  ...
  deploy. **`packageManager` is the sole authority for pnpm _in CI_** — corepack
  ...
  _different fields_, and `engines.pnpm` must be an **exact** version **equal
