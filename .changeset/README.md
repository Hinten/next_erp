...
Apps (`apps/web`, `apps/integrations`, `apps/webchat`, `apps/docs`), tooling (`tools/test-fixtures`), and shared configs (`packages/config-*`) are listed in `config.json` under `ignore` — they are private and never published to npm.
