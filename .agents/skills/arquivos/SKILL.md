...
The deep _why_ and the full lifecycle diagrams live in
...
the _how-to-use / how-to-reuse / how-to-extend_ guide — it points at those, it
...
- **Doc-anchored** — the `arquivos` Firestore doc is the source of truth, _not_
...
- **Create-first** — the client writes the doc _before_ uploading the bytes, so a
  dead upload leaves a detectable _phantom doc_, never an orphan object.
...
**Staged deletion** (mandatory convention — `apps/web/AGENTS.md` rule 7):
...
at save time. The `arquivos` doc + Storage object are reaped _after_ the produto
...
The produto-_delete_ cascade (`onDocumentDeleted('produtos/{id}')`, #136) and a
...
- `apps/functions/AGENTS.md` — operational notes for the 5 functions + deploy gotchas.
