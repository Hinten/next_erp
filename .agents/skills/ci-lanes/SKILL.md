...
root `AGENTS.md` carries only the rules you must not break.
...
1. **A non-matching top-level `paths:` publishes NO check run.** Not a failure,
   not a skip — _nothing_. The row simply is not on the PR page. So a correct
...
a workspace outside the closure, and a short inert list (`*.md`, `.Codex/`,
...
A lane's own workflow still triggers it; another lane's does not. **An empty`--self` falls back to running\*\*, so forgetting the flag can only cost an extra
...
`if: needs.changes.outputs.run_e2e == 'true'` to a _downstream_ job makes it run
...
- `pull_request: branches: [master, main, production, 'Codex/**', 'feat/**', 'fix/**']`
...
_any_ provider — anyone with write access could `POST …/statuses/<sha>` and turn
...
1. **turbo's `^build` edge on `lint`/`typecheck`/`test` resolves to ZERO tasks.**
   ...
   `0 cached, 10 total` _after_ typecheck, lint and test had run in the same job,
...
| `pnpm --filter <pkg with no "test"> run test`               | _(nothing)_                         | **0** |
...
`pnpm --filter` step, and the earlier claim that only a bad _name_ was unsafe there
...
says nothing about _what_ is being sharded.
...
"`pnpm --filter` exits 1" property. The name-check does still catch a bad _name_, so
that job is not currently broken — but the stated reasoning no longer holds, and a
renamed _script_ there would exit 0 silently. Worth revisiting on its own.
...
workflow YAML is read from the merge ref while the _script it invokes_ comes from
...
which lives _inside_ a `required_status_checks` rule — and `protect-main`
