...
"fix" this back to persisting every event — see _Why not persist-first_ below.
...
core's own catches narrow on bare `Error` _by design_ (documented at the top of
...
failure is persisted so the sweep can re-drive it. But if that persist _also_
...
3. **Index** — add to `firestore.indexes.json`:
   ```json
   {
     "collectionGroup": "notificacoes<Canal>",
...
4. **Adapter** — `apps/<canal>/lib/<dominio>/notificacao.ts`: a
   ...
   shared `asInt`/`asMillis` and spread the rest, so the parse is _total_ for
...
8. **Docs** — the app's `AGENTS.md`, and its `functions/DEPLOY.md` with the
...
for a new channel is _observe rather than guess_, and then delete the probe once
it has answered.

**Adding a receiver for a new channel?** Establish the signature posture from the
provider's _notification_ reference, not its security-recommendations page — the
