...
- **The vendored NT PDF does NOT contain these tables.**
  ...
  only**; its own **page 82** lists Anexo I/II as _"Tabela a ser publicada"_ and
...
- **UI (the shared `components/imposto/RtcSection`, behind produto + operação +
  ...
  **Free-entry is preserved** — any code not yet seeded can be typed;
  ...
  `not-in-table`). Table _membership_ is **never** an emit-time block.
...
(Anexo II, _a ser publicada_) and `cCredPres` (Anexo IV, crédito-presumido
...
- **Anexo I** — NCM do Imposto Seletivo (NCMs sujeitos a IS). _"Tabela a ser publicada."_
- **Anexo II** — Tabela `cClassTribIS` (classificação do IS). _"Tabela a ser publicada."_
...
- **Codes are validated, not pure free-text (#333)** — a **verified seed** of
  ...
  - picker (#333)".
- **Live homologação proof**: `test/operations/rtc.homologacao.test.ts` (serie 4) emits a CRT=1 NF-e with the RTC groups against SEFAZ-SP homologação and
