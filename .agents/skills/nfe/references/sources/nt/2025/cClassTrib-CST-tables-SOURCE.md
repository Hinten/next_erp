...
published by SEFAZ as **`.xlsx` spreadsheets**, updated _outside_ the NT cycle,
...
versioned" convention). This file records _where_ they are, _that they're
published_, and the _confirmed key codes_, so the update-watch routine knows
...
| NCM do Imposto Seletivo            | I     | _"A ser publicada"_ (p.82)           | idem (quando sair)                      |
| `cClassTribIS` (Imposto Seletivo)  | II    | _"A ser publicada"_ (p.82)           | idem (quando sair)                      |
...
- **`cClassTrib = 000001`** → _"Situações tributadas integralmente pelo IBS e
  CBS"_, under **`CST = 000` (Tributação integral)**. SVRS lists `000001` as the
