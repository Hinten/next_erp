...
see the `arquivos` skill and `apps/functions/AGENTS.md`), which does not need
...
v8 — see root `AGENTS.md` rule on the admin floor.
...
nested `db.pipeline()...` is embedded as an _expression_ (not a top-level stage)
...
**Scope semantics** (the trap): a **field** (`field('x')`) is _local_ to the row
the subquery is currently processing — an undefined field evaluates to **absent**,
no error. A **variable** (`variable('x')`) is _global_ to the pipeline and all
...
**whole** query including joined docs, so `select`/`where`/`limit` _inside_ the
...
a classic query — Firestore _Enterprise_ edition auto-creates **zero** indexes,
...
fetchCandidatesViaPipeline, resolveReferenced = ...)` — the emulator suite calls
  ...
  the _rest_ of the suite is emulator-only — don't let one pipeline-dependent
...
- **Search across a directory-shaped field** — §3's snippet (`filepath` regex
  ...
  couldn't) — see `apps/functions/AGENTS.md` — but the snippet remains the right
...
- **Always feature-detect, don't assume.** `isPipelineSupported(db)` checks
  ...
  Catch _only_ that specific error type to fall back to `buildQuery` — anything
...
- **Stages may be reordered by the optimizer; result order is unstable without
  `sort`.** The chained stages only guarantee the _result_ equals in-order
...
- **Absent field vs undefined variable.** A missing `field(...)` is _absent_ (safe,
