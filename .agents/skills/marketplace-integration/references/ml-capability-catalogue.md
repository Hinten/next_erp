...
deliberate _absence_ of a transaction (OCC would re-fire a non-idempotent grant) and
...
- **Completeness**: `assertOrderItemsComplete` throws `OrderItemsIncompleteError`
  ...
  answers **206 Partial Content** for a still-materialising order, _omitting_ fields.
...
- `pedidoTravadoSweep` (weekly) is the only _time-based_ release, and it is mostly a
...
The mass import is the _job_ pattern worth reusing: one durable job doc as the single
...
the family only when _every observed_ member is closed, or a sweep silently drops a
...
⚠️ Gate 5 is a live-provider trap worth generalizing the _habit_ from: a price-ONLY
...
report row stores the _code_ and renders the message at read time, so fixing a message
...
a UI hint, stale by construction; the send path re-reads and _that_ read is the
...
⚠️ `claimResolve.ts` deliberately holds **no `db` handle**, and that absence _is_ the
