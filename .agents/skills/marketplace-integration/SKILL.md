...
| ------------------------------------- | ------------------------------------------------------------------------------------- |
| _What does this channel support?_     | `MARKETPLACE_TIPO_CAPS` (`@delfrance/schemas`)                                        |
| _What shapes cross between channels?_ | `@delfrance/core/marketplace` (incidents; the order model is research, unimplemented) |
| _How does it behave?_                 | `apps/<channel>` + the shared seams below                                             |
| _How do I build it?_                  | this skill                                                                            |
...
with a citation per answer, and an explicit _unknown_ wherever the docs do not say.
...
3. **How does stock go out?** One call per listing, a batch, or an async feed? See
   _The stock chapter_ — this is the single biggest cost decision.
...
- ⚠️ Ask before opening GitHub issues (root `AGENTS.md`) — the tracker is curated.
...
Read the ML implementation for _what problems exist_, not for _what to write_. These
...
- **`pedidos/{id}/orderML`** — a byte-faithful mirror of ML's order payload, keyed by
  ...
  **no corpus to be compatible with**. Do not invent a mirror by analogy. Ask what
  the collection-group resolver is actually _for_ — "which pedido owns this provider
...
- **The single-use rotating refresh token** and its loser-fallback double re-read —
  ML's rotation _is_ the concurrency arbiter, which is why the refresh deliberately
...
outright — ML's inability _"was a per-channel limitation, **not** a property of
virtual kits, and it must not be generalized into one"_ — and #1087 is the oversell
...
These were extracted _because a second consumer needed them_. Using them is not
...
`findOrCreateCliente` is also the model for _when_ to extract: it was promoted out of
...
**Enterprise bills data scanned** (root `AGENTS.md` rule 1) — an unindexed predicate
...
contract is _"deterministic outcomes RETURN, transient failures THROW"_, and it has
...
_read_ cost is unchanged, so the tiers may still be justified by scan cost alone.
...
- **Every write can lose a race** (root `AGENTS.md` rule 7). Pick the cheapest tier
...
- Tests, including a **near-miss** wherever something decides that two values are
  "the same" — a pair that must be equal _and_ a pair that must stay distinct.
...
- ⚠️ Anything that must be _run_ against production data or infrastructure is not
  yours to do and is not a TODO: surface it and stop (root `AGENTS.md` rule 8).
