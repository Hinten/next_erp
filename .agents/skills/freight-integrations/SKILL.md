...
another aggregator) slot in by **category** — see _Provider categories_ below.
...
`FreightHttpClient` + `FREIGHT_TIPO_CAPS` (see _Provider categories_).
...
descriptive for the _marketplace_ categories; it is `'generic'` that is
load-bearing.
...
- **`tokenMelEnv` is single-token** — a refresh deletes the old token doc and
  ...
  _per ME API call_, so one `comprar` re-enters it several times), so #966 added
  ...
  **twice**, the second time after `LOSER_REREAD_DELAY_MS` (250 ms), because the
  loser learns it lost _before_ the winner's `save()` commits. Every
...
insert. Without it ME returns an **opaque HTTP 500** with _no_ validation hint —
