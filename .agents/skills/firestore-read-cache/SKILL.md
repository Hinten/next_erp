...
scanned**, and several of the reads we repeat are _queries_, not document gets. A
...
2. **Read-modify-write.** Any value the caller conditionally writes back: the
   ...
   `apps/mercado-livre/lib/marketplace/importacao/import.ts`, which _creates_ the produto when
...
3. **OAuth credentials / tokens.** `apps/mercado-livre/lib/marketplace/core/tokenStore.ts`
   ...
   _because_ they are fresh: the re-check honours a refresh that landed mid-flight,
...
means repeated _within one warm instance's TTL window_ — a once-a-day sweep gains
...
const id = await integracaoByUserId.get([userId], () => resolveIntegracaoByUserId(db, userId));
```
...
Instances do not coordinate. **The TTL _is_ the staleness bound**: after a write, a
...
`null` produces `{ kind: 'no-account' }`, which is a _deterministic ack_. Cache that
...
Reach for it when _one field_ has a tighter freshness requirement than the document
