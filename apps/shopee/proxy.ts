...
// apps/web browser, which is on a different origin (dev: :3000 → :3009; prod:
// app-* → shopee-*). They carry `Authorization: Bearer <idToken>`, which makes
// them non-simple cross-origin requests — so the browser fires a CORS
// preflight. Without an OPTIONS handler + ACAO header on the response, the
// fetch surfaces as a CORS error. The Shopee OAuth **callback**
// (/api/oauth/shopee/callback) is a top-level browser redirect from Shopee + a
// server→Shopee token exchange — no preflight — so it stays OUT of the
// matcher, along with the future push receiver (/api/webhooks/shopee,
// server→server).
