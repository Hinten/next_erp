 ...
 *
 * ## The three link folds live in `../core/vinculosShopee`
 *
 * Which conta a `prodshopee` belongs to, which LISTING a `variashopee` belongs
 * to, and which `model_id` readings mean "no usable model" — {@link idDoRef},
 * {@link varLinksDoAnuncio} and {@link modelosUtilizaveis}. They were private
 * here until step 13's price planner needed the same three answers; that
 * module's header states what each fold treats as equal and what it keeps
 * distinct. This planner IMPORTS them and holds no copy.
 *
...
export type { MembroDaFamilia, VarLinkShopeeCru };
...
export interface LinkShopeeCru extends LinkParaEstoque {
  /** Which conta owns this listing. Either stored ref encoding; see `idDoRef`'s fold (1). */
