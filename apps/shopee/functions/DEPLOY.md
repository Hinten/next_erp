...
## Runtime env (step 4's two valves + step 8's three)

None of them is a secret and none belongs in Secret Manager: they are operator
switches read with `process.env.X === '1'` at the use site (the exception is
step 8's horizon, which is a number), exactly like
...
silently. The root `.env.example` names all five anyway, so the one file
...
(#1534). ⚠️ There are now **two queue functions**, and the per-service grant
below has to be repeated for each: a grant on one of them buys nothing for the
other, and the failure is the silent one (task created, dispatched,
...
#
# ⚠ ONCE PER QUEUE FUNCTION - since step 9 there are TWO.
for fn in processShopeeNotification processShopeeMassImport; do
...
### ⚠️ Since step 4 the SCHEDULED functions enqueue too (#1512), and since step 9 a queue function enqueues ITSELF (#1517)

`sweepShopeeLostPushes` and `backfillShopeeOrders` both enqueue onto
`processShopeeNotification`, and `processShopeeMassImport` re-enqueues onto its
OWN queue for every scan/drain continuation and for the rate-limit pause — so
the **functions runtime service account** — not
just the App Hosting one — needs `roles/cloudtasks.enqueuer` on **both** queues
and `roles/run.invoker` on **both** Cloud Run services. The mass-import function
...
than leaving a `running` document that nothing will ever drain. Verify BOTH
