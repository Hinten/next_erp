...
// Bundle the Shopee Cloud Functions (codebase `shopee`) into a single
...
export async function bundle(outfile) {
  ...
  loadBuildEnv();
  // Must match the enqueuer's region (lib/shopee/shopeeTasks.ts) or tasks target
  // a queue that does not exist and are silently dropped. No default: an unset
  ...
  await build({
    ...
    banner: {
      js: "import { createRequire as __shopeeCreateRequire } from 'node:module';\nconst require = __shopeeCreateRequire(import.meta.url);",
    },
  });
  return region;
}
