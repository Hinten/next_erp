/**
 * Bearer-token verification for callable-style routes in apps/mercado-pago.
 *
 ...
 *
 * Per-app copy of the same helper apps/mercado-livre and apps/melhor-envio each
  ...
  try {
    ...
    if (e instanceof Error && typeof (e as { code?: unknown }).code === 'string') {
      ...
      if (code.startsWith('auth/')) {
        console.warn(`[mercado-pago/auth] verifyIdToken rejected: ${code}`);
        return { error: authError(401, { error: `Token inválido ou expirado (${code}).`, code }) };
      }
      console.error(`[mercado-pago/auth] admin init failed: ${code} - ${e.message}`);
      return {
        error: authError(500, {
