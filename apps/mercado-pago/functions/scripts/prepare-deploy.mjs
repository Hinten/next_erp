...
// apps/mercado-pago/functions/scripts/prepare-deploy.mjs`) from the repo root;
// firebase.mercado-pago.deploy.json points `source` at the generated folder.
// Same mechanics as apps/mercado-livre/functions (esbuild bundle + minimal
// package.json + node_modules junction), adapted marketplace → payments.

// apps/mercado-pago/functions (this script lives in .../functions/scripts/).
const pkgDir = dirname(dirname(fileURLToPath(import.meta.url)));
const repoRoot = resolve(pkgDir, '..', '..', '..');

// Artifact at <root>/.deploy/mercado-pago-functions — the SAME directory depth
// as <root>/apps/mercado-pago, whose node_modules the junction below points at
// (apps/mercado-pago/functions has no own node_modules; it is part of
// @delfrance/mercado-pago-app). pnpm's RELATIVE symlinks resolve through the
// junction only at that matching depth.
const deployDir = join(repoRoot, '.deploy', 'mercado-pago-functions');
...
if (existsSync(realNodeModules)) {
  ...
  console.warn(
    'warning: apps/mercado-pago/node_modules not found — run `pnpm install` before deploying',
  );
}
