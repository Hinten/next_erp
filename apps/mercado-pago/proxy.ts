...
// The callable payment endpoints (/api/payments/*) are invoked from the
// apps/web browser, which is on a different origin (dev: :3000 → :3007; prod:
// app-* → mercado-pago-*). They carry `Authorization: Bearer <idToken>`, which
...
// surfaces as a CORS error. The Mercado Pago OAuth **callback**
// (/api/oauth/mercado-pago/callback) is a top-level browser redirect from MP +
// a server→MP token exchange — no preflight — so it stays OUT of the matcher,
