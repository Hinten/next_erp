...
function req(body: unknown, headers: Record<string, string> = {}): Request {
  return new Request('http://localhost:3001/api/freight/melhor-envio/imprimir', {
    method: 'POST',
...
beforeEach(() => {
  ...
  h.loadCtx.mockResolvedValue({ intFreteId: 'int-1', api: { print: h.print } });
});

describe('POST /api/freight/melhor-envio/imprimir', () => {
  ...
  it('returns 200 with the label URL', async () => {
    h.verifyIdToken.mockResolvedValue(READER);
    const res = await POST(req(VALID_BODY, { authorization: 'Bearer t' }));
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ url: 'https://sandbox.melhorenvio.com.br/imprimir/abc' });
    expect(h.print).toHaveBeenCalledWith(['label-1']);
  });
});
