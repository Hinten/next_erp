/**
 * `POST /api/freight/melhor-envio/rastrear`
 *
 * Returns Melhor Envio's tracking payload for a label (keyed by order id).
 * Read-only (`PERM.frete.read`). Body `{ intFreteId, printLabelId }`.
 */
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const tracking = await ctx.api.tracking([printLabelId]);
