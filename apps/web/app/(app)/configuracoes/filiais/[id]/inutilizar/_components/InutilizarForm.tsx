...
/**
 * Inutilização de numeração form for a SINGLE filial (fixed by the route).
 * Burns a contiguous range of NF-e números on the filial's série
 * (NfeInutilizacao4) — for números that will never be authorized (gaps).
 * react-hook-form + Zod resolver (apps/web rule #4).
 *
 * Reached from the filiais list action "Inutilizar numeração" (the filial is
 * already chosen there), mirroring the old Flutter `filiaisTableView` action
 * → `InutNFeTable(filialUid)`. The burned-ranges history for this filial is
...
export function InutilizarForm({ filialId }: { filialId: string }) {
  ...
  return (
    <Stack gap="xl">
      ...
      <InutilizacaoHistory filialId={filialId} />
    </Stack>
  );
}
