...
6. **This app is SINGLE-TENANT — no query filters by tenant, and none should start.** ADR 0016 closed the multi-tenancy question (issue #14) as won't-fix: there is one tenant, no per-document `grupoEconomico` field, no path prefix, no second database. `useTenant()` reads the `grupoEconomico` custom claim and `useGrupoEconomico()` subscribes to `grupoEconomico/<claim>`, but the **only** thing either feeds is the `TenantBadge` header display and the configurações card — the surface is vestigial and deliberately kept, not a scoping mechanism. ⚠️ This line used to read *"All queries filter by it"*, which had been **false** since the User+Cargo review PR stripped the field: `cargos`/`usuarios` are unfiltered, the `callerGE` check in the admin users endpoints is gone, and the single generated rule (`EXTRA_MATCH_BLOCKS` → `match /grupoEconomico/{grupoId}`) only lets you read your **own** tenant doc. Access control here is permission bits in Firestore rules and nothing else. A second tenant needs a **new ADR before any code**, not a filter added by hand.
