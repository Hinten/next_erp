...
| `sweepMercadoLivreStockDaily`        | `onSchedule('7 2 * * *')`              | Step 10 PR C — the **daily** full-reconciliation sweep, 02:07 America/Sao_Paulo: the same discovery over a flat 24h window (`MERCADO_LIVRE_STOCK_DAILY_WINDOW_H`), with no activity filter and no pedidos probe. The `:07` offset keeps it off the 15-minute tick's `:00/:15/:30/:45` slots so the two never contend for one conta's caps and state doc. Same flag, same no-op while OFF.                                                                                                                                                              |
