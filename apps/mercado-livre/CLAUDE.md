...
**publish**, the **UP member publish** and the **stock send** — the last on BOTH its
success writeback and its terminal-4xx verification path, which reads a real
`GET /items` and so holds the stronger evidence of the two. Six writers, one
