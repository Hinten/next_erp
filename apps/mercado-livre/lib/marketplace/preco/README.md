...
⚠️ **The backend owns the operator-facing wording.** Each route returns a pt-BR
`mensagem` per listing and the caller passes it through verbatim; a second copy
of the skip vocabulary would drift from the gates it describes.
