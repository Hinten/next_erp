...
vi.mock('../importacao/importMigration', () => ({ handleUptinMigration: h.handleUptinMigration }));
vi.mock('../pedidos/orderImport', () => ({ importPedidoMercadoLivre: h.importPedidoMercadoLivre }));
vi.mock('../pedidos/orderPaymentImport', () => ({
  importPagamentoMercadoLivre: h.importPagamentoMercadoLivre,
}));
vi.mock('../pedidos/orderShipmentImport', () => ({
  importShipmentMercadoLivre: h.importShipmentMercadoLivre,
}));
vi.mock('../claims/claimImport', () => ({ importClaimMercadoLivre: h.importClaimMercadoLivre }));
...
describe('handleNotificationTask', () => {
   ...
   *
   * `done` is a disposition, not a claim that work happened: an items sync that
   ...
   * on both an ignored and a performed sync, rather than trusted.
   */
  it('carries the items-sync outcome out on `detail` — a no-link sync is not a silent success', async () => {
    const db = new FakeDb();
    seedConta(db, 'conta-A', 55);
    const r = await handleNotificationTask(
      asDb(db),
      payloadOf({ id: 'N10b', resource: '/items/MLB404', topic: 'items' }),
      0,
      { resolveItemsApi: vi.fn(async () => ({ getItem: vi.fn() }) as never) },
    );
  ...
  it('reports the `kind` even where there is no detail — an ignored topic is not a processed one', async () => {
    ...
    const r = await handleNotificationTask(
      ...
      payloadOf({
        id: 'N10d',
        resource: '/sites/MLB/user-products-families/1',
        topic: 'user-products-families',
      }),
      0,
      {},
    );
    expect(r).toMatchObject({ kind: 'ignored-topic' });
...
describe('handleNotificationTask — post_purchase claim dispatch (#1322)', () => {
  ...
  it('a resource naming no claim at all is still malformed — dropped without dispatching', async () => {
    const db = new FakeDb();
    seedConta(db, 'conta-A', 55);
    const claimImportRunner = claimRunner();
    const r = await handleNotificationTask(
    ...
    expect(r).toMatchObject({ outcome: 'dropped' });
    expect(claimImportRunner).not.toHaveBeenCalled();
  });
});
