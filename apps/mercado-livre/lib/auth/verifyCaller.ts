/**
 * Bearer-token verification for callable-style routes in apps/mercado-livre.
 *
 ...
 *
 * Per-app copy of the same helper apps/melhor-envio and apps/nfe each carry —
...
export async function verifyCaller(
  ...
  try {
    ...
    if (e instanceof Error && typeof (e as { code?: unknown }).code === 'string') {
      ...
      console.error(`[mercado-livre/auth] admin init failed: ${code} - ${e.message}`);
      return {
        error: authError(500, {
